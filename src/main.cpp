...
public:
    ...
    bool on_init() override {
        if (src) {
            ...
            std::string cmd = "gcc -g -Wall -Wextra " + out + "/*.c -o " + out + "/executable -I " + (program_location / "include/ouverium").string() + " -Wl,-rpath," + program_location.string() + "/build build/libcapi.so";
            system(cmd.c_str());
        } else {
